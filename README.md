...
Ranked-choice Voting is a Flask app that serves API endpoints for ranked-choice voting, supporting both creations of elections, retrieval of results, and casting of votes using HTTP requests.
...
graph TD
    ...
    B --> C{Does the first choice have<br>simple majority?}
    C --> |Yes| D[The first choice wins]
...
A `GET` request is the quickest way to set up a ranked-choice election but offers no customization options since all fields take their default values. Create an election by appending a `/` separated list of candidates to the `/add` endpoint. 
...
| `end_time`        | Yes       | 7 days after `start_time`             | The timestamp at which your election ends. Votes made only before the end time will be counted.       |
| `description`     | Yes       | NULL          | A short description of your election                                                               |
...
You can cast your votes by sending a `GET` request to the `vote/election_id` endpoint. Append the URL with the ordered candidates, separated with a ```/```. Remember to include all the candidates since this is ranked-choice voting.
...
3. Modify the `.env` file and add your database connection URL. For example, if you have set up PostgreSQL locally, you can use the following:
    ```bash
...
The app uses IP addresses to track different users. This is used to map a user to their vote and is used to update votes (or prevent multiple votes). Every time you create an election or add a vote, your IP address is recorded and used to identify you.

### Can I update my vote?

This behavior depends on the `update_votes` field. If this field is set to `True`, then you can update or remove your vote from the election.
